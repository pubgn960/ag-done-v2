...
def parse_order_v2(
    ...
    # Unlabeled Credential Inference (when email or phone is present but password/username have no explicit labels)
    identifier_str = email or phone
    if identifier_str:
        id_line_idx = -1
        for idx, line in enumerate(lines):
        ...
        if id_line_idx != -1:
            # Unlabeled password: line after identifier if not ignored and not package/2FA
            if not password:
            ...
            # Unlabeled username: line before identifier if not ignored and not customer_ref
            if not username:
