...
async def setbinance_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Client command /setbinance [binance_uid].
    Registers or updates client's Binance UID for the current Category B group.
    """
    user = update.effective_user
    chat = update.effective_chat
    if not user or not chat:
    ...
    category = CLIENT_GROUPS_CACHE.get(chat.id, "A")
    if category != "B":
        await update.message.reply_text("ℹ️ Binance wallet identity registration is active only for Category B groups.")
        return

    args = context.args or []
    if not args:
        msg = (
            "⚠️ <b>Register Binance UID</b>\n\n"
            "Please provide your numeric Binance UID.\n"
            "Usage:\n"
            "<code>/setbinance &lt;your_binance_uid&gt;</code>\n\n"
            "Example:\n"
            "<code>/setbinance 123456789</code>"
        )
        await update.message.reply_text(msg, parse_mode="HTML")
        return

    binance_uid = args[0].strip()
    if not binance_uid.isdigit() or not (5 <= len(binance_uid) <= 20):
        await update.message.reply_text("❌ Invalid Binance UID. Please provide a valid numeric Binance UID (e.g. 123456789).", parse_mode="HTML")
        return
    ...
    confirm_msg = (
        f"⚠️ <b>Confirm Binance UID Registration</b>\n\n"
        f"<b>Group:</b> {html.escape(group_name)}\n"
        f"<b>Customer:</b> {html.escape(user_name)} (<code>{user.id}</code>)\n"
        f"<b>Binance UID to Link:</b> <code>{html.escape(binance_uid)}</code>\n\n"
        f"Please confirm that this is your correct Binance UID."
    )
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Confirm", callback_data=f"setbin_confirm:{binance_uid}"),
            InlineKeyboardButton("❌ Cancel", callback_data="setbin_cancel")
        ]
    ])
    await update.message.reply_text(confirm_msg, reply_markup=keyboard, parse_mode="HTML")


async def setbinance_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handles confirmation / cancellation for Binance UID registration."""
    query = update.callback_query
    if not query:
        return
    await query.answer()
    ...
    category = CLIENT_GROUPS_CACHE.get(chat.id, "A")
    if category != "B":
        await query.edit_message_text("ℹ️ Binance wallet identity registration is active only for Category B groups.")
        return

    data = query.data or ""
    if data == "setbin_cancel":
        await query.edit_message_text("❌ Binance UID registration cancelled.")
        return

    if data.startswith("setbin_confirm:"):
        binance_uid = data.split(":", 1)[1]
        identity, success, reason = await register_binance_identity(
            client_group_id=chat.id,
        ...
        if success:
            group_name = chat.title or str(chat.id)
            succ_msg = (
                f"✅ <b>Binance Identity Linked Successfully!</b>\n\n"
                f"<b>Group:</b> {html.escape(group_name)}\n"
                f"<b>Telegram User:</b> <code>{user.id}</code>\n"
                f"<b>Linked Binance UID:</b> <code>{html.escape(binance_uid)}</code>\n"
                f"<b>Status:</b> ✅ Linked\n\n"
                f"Payments received from Binance UID <code>{html.escape(binance_uid)}</code> will automatically credit your wallet in this group."
            )
            await query.edit_message_text(succ_msg, parse_mode="HTML")
        else:
            await query.edit_message_text(f"❌ Failed to link Binance UID: {reason}")


async def binanceid_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Admin command /binanceid or /binanceusers.
    Lists registered Binance identities for the current Category B group.
    """
    user = update.effective_user
    chat = update.effective_chat
    if not user or not chat or not (is_super_admin(user.id) or is_admin(user.id)):
        logger.warning(f"[BINANCE_ADMIN] Unauthorized /binanceid attempt by user #{user.id if user else 'Unknown'}.")
        return

    category = CLIENT_GROUPS_CACHE.get(chat.id, "A")
    if category != "B":
        await update.message.reply_text("ℹ️ Binance wallet identities exist only for Category B groups.")
        return

    identities = await get_all_group_binance_identities(client_group_id=chat.id)
    group_name = chat.title or str(chat.id)

    lines = [f"📊 <b>Category B Registered Binance Clients</b>\n"]
    lines.append(f"<b>Group:</b> {html.escape(group_name)}\n")

    if not identities:
        lines.append("<i>No Binance clients registered for this group yet.</i>")
    else:
        for idx, (ident, _) in enumerate(identities, 1):
            lines.append(
                f"{idx}. 👤 <b>Customer</b>\n"
                f"Telegram ID: <code>{ident.telegram_user_id}</code>\n"
                f"Binance UID: <code>{html.escape(ident.binance_uid)}</code>\n"
                f"Status: ✅ Linked\n"
            )
        lines.append(f"<b>Total Registered Clients:</b> {len(identities)}")
